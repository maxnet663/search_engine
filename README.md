...
3. Оба скрипта принимают единственный параметр, а именно: директорию установки. На windows
...
| `void`                     | `updateConfig(const std::string &path = "")`                  | Обновляет текущий config.json файлом переданным по пути path. Если передана пустая строка, объект заново откроет и прочитает содержимое текущего файла                                                                                                                                                                            |
| `void`                     | `updateRequests(const std::string &path = "")`                | Обновляет текущий requests.json файлом переданным по пути path. Если передана пустая строка, объект заново откроет и прочитает содержимое текущего файла                                                                                                                                                                          |
