  ...
  cmakelist'e, расскоментируйте и укажите предпочтительный каталог. Если собираете вручную:
    ...
    * Перейдите в созданную директорию с помощью команды 
      `cd build`, затем пишем  
         ...
         генератоh файлов, например `cmake -G "Unix Makefiles"`. Если что-то пойдет не так, не переживайте,
...
````
├── install/path
│── search_engine <--------- файл запускате эмуляцию сеанса работы с поисковым движком
│── search_engine_simple <-- выполнение простого поиска и выдача результата в answers.json
...
эти файлы к текущем дереве, поэтому они должны находиться в его досягаемости. Результат поиска записывается в
...
для поиска json файлов, если такой пут не передан, использует текущий каталог.
