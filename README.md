...
````
├── install/path
│── search_engine_shell <--- файл запускает эмуляцию сеанса работы с поисковым движком
...
search_engine_shell - предлагает работу с поиском через терминал.

search_engine принимает до двух параметров. Примеры вызова:

`./search_engine /path/to/find/jsons` - первый параметр интерпритируется как директория для поиска конфигурационных файлов

`./search_engine /path/to/config.json /path/to/requests.json` - принимает пути к конф. файлам в явном виде. 

Эмулятор сеанса работы запускается без параметров, оболочка попытается обнаружить json файлы автоматически и в случае неудачи
пригласит вас к их созданию или обнаружению.
В
одном с и затем дав команду обновления можно выполнить несколько запросов подряд редактируя файлы json прямо в
время исполнения и затем дав команду обновления. Подробнее можно узнать здесь: [comands](#commands-list)
...
`class Shell` Предоставляет интерфейс для иммитации сеанса работы с сервером.

У объекта перегружен оператор вызова функции `operator()`, который и создает сессию работы с пользователем.
#### Commands list:
| Command                | Description                                         |
|------------------------|-----------------------------------------------------|
| `update-db` or `up-db` | updates data base if you changed indexed docs       |
| `update-rq` or `up-rq` | updates requests if you changed  requests.json      |
| `help` or `h`			       | prints the list of commands                         |          
| `status` or `stat`     | prints info about session                           |
| `find` or `f`          | searches for current queries in the current database|
| `print-rq ` or `p-rq`  | prints current requests from requests.json          |
| `print-db ` or `p-bd`  | prints current indexed docs from config.json        |
| `print-ans` or `p-ans` | prints cuurent search results from answers.json     |
